...
def main():
    ...
    if not h_path.exists():
        ...
        print("Training may strictly speaking not have started, or incorrect path.")
        return
    ...
    if n >= 10:
        ...
        if late_avg < early_avg * 0.5:
            print("  [OK] Loss consistently decreasing")
        elif late_avg < early_avg:
            print("  [WARN] Loss decreasing but slowly — may need more time")
        else:
            print("  [FAIL] Loss not decreasing — potential problem")
    ...
    if n >= 10:
        ...
        if last_val > last_loss * 5:
            print(f"  [WARN] Large val/loss gap ({last_val:.3f} vs {last_loss:.3f}) — watch for overfitting")
        else:
            print(f"  [OK] Val/loss ratio healthy ({last_val:.3f} / {last_loss:.3f})")
    ...
    if ema_maes and len(val_maes) > 0:
        ...
        if best_ema < best_reg:
            ...
            print(f"  [OK] EMA is helping: {best_ema:.4f} vs {best_reg:.4f} ({pct:.1f}% better)")
        else:
            print(f"  [INFO] EMA not better yet (reg={best_reg:.4f}, ema={best_ema:.4f})")
